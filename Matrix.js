...
//perc = 1 means n will be a
Matrix.mix = function(a, b, perc) { // returns a new matrix with a perc mix of a:b
    var n = new Matrix(a.rows, a.cols);
    for (let i = 0; i < a.rows; i++) {
        for (let j = 0; j < a.cols; j++) {
            var r = random(1);
            if (r < perc) {
                n.matrix[i][j] = a.matrix[i][j];
            } else {
                n.matrix[i][j] = b.matrix[i][j];
            }
        }
    }
    return n;
